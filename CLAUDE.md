...
- **Deploy = `make build`.** Plus `make build-lambdas` + `km init --dry-run=false` for the ttl-handler's two new exact-name `ssm:DeleteParameter` ARNs — `TestTTLHandlerModule_EveryCleanupSSMParamHasADeleteGrant` (`cmd/ttl-handler`) runs the real `CleanupSandboxIdentity` against a recorder and pairs every path it deletes with a module ARN, verified to fail with one removed. No userdata, schema, sidecar, or recreate. `km doctor` gains a WARN for orphan `/{prefix}/access/*` params. Plugin `0.4.17`. See `docs/vscode.md` § Sharing a sandbox between analysts.
