...
**Hibernate-safe additional volumes — `km-volumes` (2026-09-20):**
- **A hibernate/resume can cross-wire the additional NVMe namespaces; `/etc/fstab`'s mount-by-UUID then writes both volumes onto the wrong disk.** Reproduced on cycle 1 of `km pause`/`km resume` on a two-volume t3.medium (`sb-3cf8e982`): the kernel re-reads namespace SIZE but keeps cached controller IDENTITY and page cache, so `/sys/block/*/device/serial` and `blkid` both lie after resume — **only a live `nvme id-ctrl` Identify tells the truth** (it showed the two volumes had actually swapped controllers). The operator write-up's "serials were right" was sysfs. A 1 MiB known file failed its checksum 33 s after resume.
- **Fix = new sidecar `cmd/km-volumes` + userdata.** The fstab lines are gone. First cold boot writes `/var/lib/km/volumes.json` (live serial → vol-id, size, fs UUID, mountpoint, snapshot provenance). `km-volumes.service` (oneshot, before sshd/pollers) runs `mount` on EVERY boot: resolve by live serial (never BDM letter, never UUID), check size + UUID + superblock-fits, mount only on a full match, else refuse with `<mp>/.km-mount-refused` + `/var/lib/km/volumes.state` + a `volume_mount_refused` audit event. `system-sleep/km-volumes`: `pre-sleep` unmounts (TERM→KILL→lazy-with-marker, every step under `timeout`, ALWAYS exit 0 — a blocking hook trips ec2-hibinit's retry/debounce and the box stops hibernating), `post-sleep` PCI-removes + rescans every non-root controller UNCONDITIONALLY (0.7 s live; node names AND BDFs move, serial is the only key) then mounts; one retry; then `spec.runtime.onVolumeMismatch: refuse|reboot` (default refuse; reboot is once per resume, no loop). `km-volumes repair <mp>` = backup-superblock e2fsck, snapshot-derived only unless `--i-accept-data-loss`. Sidecar is fetched in userdata section 2.6.0, BEFORE the mount block — the shared sidecar download runs later in the script.
- **`km status` `Volumes:` section (one SSM probe, running boxes), `km resume` polls for it (≤240 s; a hibernation resume answers SSM at ~150–300 s, so it can still time out and says to check `km status` later), `km doctor` `Additional volumes` (latest audit event per mountpoint), `klanker:sandbox` B5 reads state + marker and is told not to diagnose a refused mount as a wrong volume.** `/shared` EFS across hibernate is a SEPARATE hang problem, deliberately untouched.
- **Deploy = `make build` + `make build-lambdas` + `km init --dry-run=false`; do not split.** Existing boxes get nothing until recreate; until then `hibernation: false` on volume-bearing profiles (idle-stop cold-boots, never affected). Frozen pre-92 learn.v2 golden was hand-patched (diff verified = exactly the three km-volumes hunks), never re-captured. Plugin `0.4.18`. See `docs/hibernate-volumes.md`.

**Shared sandbox access credentials (2026-09-20):**
- **`km vscode|desktop|herdr|tunnel start` from a laptop that never ran `km create` now just works.** The per-sandbox ed25519 key and KasmVNC `user:pass` live in SSM SecureString at `/{prefix}/access/{id}/{ssh-key,desktop-cred}` (platform KMS key), and `~/.km/keys` / `~/.km/desktop` are a CACHE reconciled by `syncSharedCredential` (`internal/app/cmd/shared_cred.go`) before every pre-flight: pull if absent, refresh if different, publish if SSM is empty (self-backfills every pre-existing sandbox), name `rekey` if neither exists, WARN-and-continue on a read error. **One shared credential, no per-analyst identity — decided, don't re-litigate:** anyone who can run `start` already holds operator creds and therefore `km shell --root`, so the key is a convenience, not a boundary.
- **NOT under `/{prefix}/sandbox/{id}/`** — the instance role has `ssm:GetParameter*` on that path. `/{prefix}/access/` is inside the operator policy's existing `parameter/{prefix}/*` grant; laptops need no IAM change.
- **`rekey` publishes box → local → SSM, last writer wins.** SSM-first would hand every laptop a key the box rejects if the push failed; this way a failed publish leaves you working, others stale, and says exactly that. The vscode and herdr pre-flights compare the box's `authorized_keys` fingerprint to the synced key and name `km vscode rekey` on mismatch (conservative: an unparseable box line is NOT a mismatch). `km tunnel` has no pre-flight, so sync but no guard. The desktop password is hashed on the box, so no guard there either — a stale one is a browser login failure.
- **Test seam:** `NewSharedCredStoreFunc` returns `(nil, nil)` in `TestMain`; a nil store is byte-identical legacy behaviour (including the old "copy the files over" error text), so every pre-existing vscode/desktop/herdr/tunnel test is untouched.
- **Deploy = `make build` — the feature is the LAPTOP's binary; a sandbox never runs km, so every existing box is in the moment a laptop upgrades.** Plus `make build-lambdas` + `km init --dry-run=false` because cleanup runs in the ttl-handler for a TTL expiry AND for a default `km destroy` (`--remote`): until the Lambda is redeployed both leave the two params behind (harmless; `km doctor` WARNs). That apply also adds the two exact-name `ssm:DeleteParameter` ARNs — `TestTTLHandlerModule_EveryCleanupSSMParamHasADeleteGrant` (`cmd/ttl-handler`) runs the real `CleanupSandboxIdentity` against a recorder and pairs every path it deletes with a module ARN, verified to fail with one removed. **Rollout order: upgrade every laptop before anyone rekeys** — rekey rotates the box and an old-binary laptop has no sync. No userdata, schema, sidecar, or recreate. Plugin `0.4.17`. Pre-existing drift found on the way: `km destroy` removes `~/.km/keys/<id>*` but never `~/.km/desktop/<id>` (docs corrected; code untouched). See `docs/vscode.md` § Sharing a sandbox between analysts and `docs/operational-gotchas.md` § Shared sandbox access credentials.
- **Same branch:** `km version` (= `km --version`, one template rendered against the root) and `km list --wide` renders an expired TTL as `exp.` — `computeTTLRemaining`'s 7-char `expired` in a `%-6s` column was shoving IDLE/UP/💬 right on that one row (`--json` keeps `expired`).
...
| **What the YAML → HCL compiler emits** — why `km-config.yaml` never reaches the compiler, the `extends` deep-merge rules and the list-union narrowing trap, `service.hcl` as a `locals` block rather than Terraform, and the `artifacts/<id>/` S3 set a `--remote` create depends on | `docs/diagrams/security/08-yaml-to-hcl-compiler.html` + `docs/diagrams/security/README.md` § YAML → HCL |
| **What `km bootstrap` vs `km init` actually applies** — the SCP unit's own provider and non-regional state key, the TWO foundation resources created by direct SDK calls (so no plan shows them and only `km unbootstrap` removes them), why the 28-unit order is load-bearing, and one `resource_prefix` traced from yaml to a module variable | `docs/diagrams/security/09-bootstrap-and-init.html` + `docs/diagrams/security/README.md` § The HCL for bootstrap and init |
...
| Hibernate/resume cross-wiring additional EBS volumes — why fstab-by-UUID corrupted them, the live-Identify rule, the pre/post sleep hooks, `onVolumeMismatch`, `km-volumes repair`, re-materialising from snapshot, the `/shared` caveat | `docs/hibernate-volumes.md` |
| Sharing a sandbox's VS Code / desktop / herdr / tunnel access between analysts — the SSM-held key + password under `/{prefix}/access/`, the sync table, rekey publish order, the mismatch guard, why the desktop has none | `docs/vscode.md` § Sharing a sandbox between analysts + `docs/desktop.md` § Sharing a desktop between analysts |
