...
- **On-box Bifrost gateway (`:8001`) is a CORE multi-provider ROUTER**, not a shim. **O7 discovery:** Codex requires the OpenAI **Responses API** (Feb 2026) and vLLM serves only Chat Completions, so codex cannot point straight at vLLM — Bifrost (Go single-binary v1.0.6; LiteLLM = documented fallback) translates Responses→vLLM, serves Anthropic Messages for Claude Code, and routes by model name: `local` (vLLM) / `claude-bedrock` (instance role, keyless) / `claude-anthropic` (`ANTHROPIC_API_KEY` via SOPS) / `gpt-oss-bedrock` (Bedrock OpenAI gpt-oss, keyless — GA Aug 2025) / `gpt-frontier` (dormant). Cloud routes flow through the MITM proxy → metered automatically. Bifrost OTLP → `:4318` (`km otel`).
