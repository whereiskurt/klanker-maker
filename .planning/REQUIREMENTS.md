...
### Phase 126 — Cross-account capacity borrowing (synthetic IDs)

| ID | Description | Status |
|----|-------------|--------|
| REQ-126-CFG | `launch_accounts` block in km-config.yaml incl. the v2→v merge-list entry (without it the block is silently dropped); `RuntimeSpec.LaunchAccount` field + JSON schema entry (`additionalProperties:false`); `km validate` rejects an unknown link name and rejects `launchAccount` + `spec.network.privateSubnet` together; no apiVersion bump | Complete (126-01) |
| REQ-126-ENROLL | `km account add`, run with target-account admin creds: launcher role + boundaried box role + `{prefix}-results-{B}` bucket (bucket-owner-enforced; B box RW / A read-only) + optional `--provision-efs`. Trust policy names all three A-side principals up front — operator SSO, `*-create-handler`, `*-ttl-handler`. `--provision-network` provisions one subnet per AZ; the link stores a subnet list, not a single id | Complete (126-04 infra, 126-05 command) |
| REQ-126-REGISTER | `km account register` / `list` / `rm`, run with home-account creds: writes the link record + SSM SecureString external id; appends the one home-side grant (`s3:GetObject` only, no PutObject) for B's box role on the artifacts bucket; `rm` removes its statement | Complete (126-07) |
| REQ-126-LAUNCH | Sandbox template gains a conditional `generate "provider"` override keyed off a `launch_account` local the compiler writes into `service.hcl`, preserving the Phase-125 verbatim-copy invariant for `terragrunt.hcl`. Network resolution sources subnets/SG/region from the link at both the local `create.go` path and the `runCreateRemote` copy | Complete (126-02 mechanism, 126-06 Go wiring) |
| REQ-126-CAPACITY | First net-new `stscreds.AssumeRoleProvider` helper in `pkg/aws`; a second assumed-role `awsCfg` so `RankAZs` and `km capacity` gate on the linked account's `L-DB2E81BA`. Capacity rows namespaced per account inside the store constructor, NOT by rewriting the `instanceType` argument. The store stays home | Complete (126-03, 126-06) |
| REQ-126-TEARDOWN | Not optional. `km destroy` assumes the launcher, including the cold-clone fallback whose synthesized `minimalHCL` now carries the launch-account locals; ttl-handler renders a conditional `assume_role` in its destroy `main.tf` with no silent home-account fallback; scoped `count`-gated IAM; `km account rm` | Complete (126-03 metadata, 126-08) |
| REQ-126-DOCTOR | Four checks: link well-formed, launcher assumable (forces `Credentials.Retrieve` so a broken trust policy cannot report healthy), artifacts grant present and not write-widened, no orphaned instances in the linked account. All skip silently when unconfigured; `--ignore-prefix` semantics unaffected | Complete (126-09) |
| REQ-126-UAT | Live: `km capacity <gpu-profile>` reads the linked account's headroom; `km create` local and remote land a GPU box there reachable via `km shell` / `km model start`; `km destroy` reaps it; a TTL expiry reaps it unattended; `simulate-principal-policy` confirms the launcher can do nothing but the bounded GPU launch | **Mostly verified live 2026-08-24; one clause BLOCKED externally.** Verified: `km capacity` reads the target account (768 vCPU vs 64 home); the `simulate-principal-policy` containment matrix (9 rows, 8 pass — see Finding L1); `km doctor`'s four launch-account checks; the read-only artifacts grant surviving `km init`; cross-account `km destroy`. **Not verified: vLLM serving a token** — every allowlisted GPU shape (`g6e.4xlarge`/`12xlarge`/`48xlarge`, `g6.12xlarge`) probed `InsufficientInstanceCapacity` in every us-east-1 AZ, so no box could launch (Finding L4). Local-path clauses NOT run — operator directed remote-only. Three defects found and written up in `126-UAT.md`: **L1** (`RunInstances` not tag-gated), **L2** (provider ICE retry defeats the AZ sweep), **L3** (`rankScore` short-circuits on success — ROOT CAUSE, confirmed experimentally). L2/L3 are Phase 124 bugs, not Phase 126. |

*Last updated: 2026-08-22 — REQ-126-CFG..REQ-126-UAT added: cross-account capacity borrowing (launch sandboxes into a linked AWS account). Backfilled at phase close; the phase's plans carried these IDs in frontmatter only.*
