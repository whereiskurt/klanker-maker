...
| BRG-01 | Slack bridge calls `quota.Record` for `ActionPost`/`ActionUpload`; 429 in BLOCK mode | unit | `go test ./pkg/slack/bridge/... -run TestQuotaRecord` | ⬜ in-plan | ⬜ pending |
...
| H1-01 | H1 bridge calls `quota.Record` for `h1_comment`; same frozen/notice path | unit | `go test ./pkg/h1/bridge/... -run TestQuotaRecord` | ⬜ in-plan | ⬜ pending |
...
| META-02 | `marshalSandboxItem` emits `action_frozen`/`frozen_*`/`action_limits` | unit | `go test ./pkg/aws/... -run TestMarshalFrozen` | ⬜ in-plan | ⬜ pending |
...
| INIT-02 | `lambdaBuilds()` includes `km-quota-alerter` | unit | `go test ./internal/app/cmd/... -run TestQuotaAlerterBuildListMembership` | ⬜ in-plan | ⬜ pending |
| CFG-01 | `limits:` km-config key NOT silently dropped (v2→v merge-list) | unit | `go test ./internal/app/config/... -run TestLimitsConfigLoaded` | ⬜ in-plan | ⬜ pending |
| PROF-01 | `spec.limits` parses + validates; JSON schema `additionalProperties:false` | unit | `go test ./pkg/profile/... -run TestSpecLimits` | ⬜ in-plan | ⬜ pending |
| CMP-01 | Compiler emits resolved limits into userdata (proxy env) + `action_limits` attr | unit | `go test ./pkg/compiler/... -run TestActionLimitsEmission` | ⬜ in-plan | ⬜ pending |
...
| CLI-03 | `km list`/`km status` render `FROZEN`; `km doctor` surfaces frozen + new table | unit | `go test ./internal/app/cmd/... -run 'TestList.*Frozen|TestDoctor.*Frozen'` | ⬜ in-plan | ⬜ pending |

*Status: ⬜ pending · ✅ green · ❌ red · ⚠️ flaky*

*File Exists legend: ❌ W0 = stub created in Wave 0 plan 121-01 (target code absent when an implementing plan first runs); ⬜ in-plan = created AND verified inside its own implementing plan's task (proper `<automated>` verify, no pre-stub needed); ✅ extend / ✅ update = the test file already exists and is extended.*
...
- [ ] `internal/app/cmd/init_plan_test.go:439` — bump hardcoded module count `24` → `26` (table + alerter, both added this phase); **must update in lockstep with the `regionalModules()` edit or the test reds on every run**
