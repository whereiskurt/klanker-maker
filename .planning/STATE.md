...
- Phase 72 added: Slack corporate-workspace support with auto-detect invite and manifest generator — install KM Slack app in a corporate workspace (e.g., Greenhouse) where most invitees are native members, while still supporting external collaborators via Slack Connect. New unified invite orchestrator in `pkg/slack/invite.go` (`EnsureMemberByEmail` returning `InvitedDirect|InvitedConnect|SkippedExternal|AlreadyMember|Failed`) tries `users.lookupByEmail` → `conversations.invite` first, prompts before falling back to `conversations.inviteShared` (interactive only; fail-fast non-interactive). New low-level methods `LookupUserByEmail` + `InviteUserToChannel` alongside existing `InviteShared`. New ad-hoc command `km slack invite <email> [--channel <name|id>] [--external]`. New profile field `spec.cli.notifySlackInviteEmails: []string` for per-sandbox auto-invite at `km create` (fail-soft: skip+warn for Connect-needed addresses since `km create` may run scheduled). New standalone command `km slack manifest` renders an embedded JSON template with deployment-specific `request_url` (bridge Lambda URL from SSM) + resource_prefix-aware display name + full scope list; output to stdout for copy-paste into Slack admin "From manifest" UI. Manifest scope additions vs current production app: `users:read.email` (required for `users.lookupByEmail`). Existing `km slack init` invite path refactored to use the new orchestrator (no behavior change for existing PoC installs). No changes to bridge Lambda, sidecars, signing, Connect transport, existing channels/SSM keys.
- Phase 73 added: km vscode remote session via SSM — new `km vscode start | stop | status` subcommand wraps SSM `systemctl restart km-vscode` + `AWS-StartPortForwardingSession` for an in-sandbox VS Code Web session. km owns the runtime contract: systemd unit `km-vscode.service`, launch wrapper at `/opt/km/bin/km-vscode-launch` (rotates token + execs `code serve-web` bound to `127.0.0.1:8443`), and token file `/etc/km/vscode/token` (mode 0600, owned by `sandbox`, regenerated every `start`). Server binds 127.0.0.1 only — SSM tunnel is the security boundary, URL token is defense-in-depth. New profile flag `spec.cli.vscodeEnabled` (default `true`) gates whether userdata writes the unit/wrapper/dir at boot. Operator's only job is installing the `code` binary in their `initCommands` (Microsoft `code serve-web` documented as the tested default; `code-server` and `openvscode-server` also supported). Userdata template change ⇒ `make build && km init --sidecars` for the management Lambda; existing sandboxes need `km destroy && km create` to pick up the unit. No new IAM, DDB schema, SSM parameters, or Lambda. Spec: `docs/superpowers/specs/2026-05-06-km-vscode-design.md`.
