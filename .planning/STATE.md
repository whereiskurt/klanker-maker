...
---
### Historical (pre-Phase-113, retained verbatim)
...
| Phase 111-rich-slack-rendering-markdown-and-table-blocks-opt-in P04 | 8min | 2 tasks | 7 files |
...
- [Phase 111]: rich-mixed corpus generated by running RenderRich then capturing output as authoritative golden; TestRichDefaultPathFrozen explicitly named for RICH-18 traceability; KM_SLACK_AI_FOOTER operator-controlled (not compiler-emitted); plugin 0.4.8→0.4.9 lockstep bump in single commit
- [Phase 113]: yaml.Marshal(p) in generateUserData() before tmpl.Execute captures all mutations; section 2.10 uses single-quoted heredoc sentinel KM_PROFILE_EOF; {{- if .ProfileYAML }} gate is non-fatal; no signature change to Compile/generateUserData; write verbatim (no redaction)
...
- Phase 111 added: Rich Slack rendering — markdown + table blocks (opt-in). Adopt Slack's `markdown` block (GA Feb 2025) + dedicated `table` block (GA Aug 2025) in a new Tier-3 `renderRich` (`pkg/slack/blocks.go`) selected by `KM_SLACK_RENDER=blocks-rich`: prose → `markdown` blocks (native GFM incl. clickable `[label](url)` link anchors — the operator's anchor ask, free; 12K cumulative cap w/ chunking); GFM pipe tables → dedicated `table` blocks via a new GFM-table→table-block transformer (alignment from delimiter row; cells raw_text/raw_number/rich_text; guards ≤20 cols / ≤100 rows / ragged-row padding; fall back to monospace reflow beyond limits); leading H1 still → `header` block (markdown-block headings flatten to one size); 🔧 tool lines stay `context` blocks; `text` fallback unchanged. `KM_SLACK_AI_FOOTER` opt-in flag (default OFF) for a "Generated by AI" context footer. Fallback chain intact: renderRich → ok=false → Tier-2 RenderBlocks → Tier-1 Mrkdwnify (existing tiers DEMOTED not deleted; default stays `blocks` ⇒ default-path goldens frozen). Decisions locked 2026-06-14: link anchors = native-only (no bare-URL auto-anchor, no Sources footer); rollout = opt-in first; AI footer = opt-in flag; streaming (chat.startStream) = deferred. Pure `pkg/slack` + `cmd/km-slack` sidecar — NO TF/DDB/SandboxProfile schema change, NO bridge Lambda change (PostMessageBlocks forwards blocks verbatim). Deploy via `km init --sidecars`; existing sandboxes recreate. Backing spec: `.planning/research/slack-markdown-block.md`. Brainstormed from operator question "why didn't we use Block Kit / leverage anchors for clicking" — research found we already use a conservative Block Kit subset; the upgrade is markdown+table blocks. Depends on Phase 110 + Phase 74 (Tier-2 renderer).
...
- Phase 121 added: Action quota + freeze quarantine for high-impact outbound actions. External, agent-untrusted quota at chokepoints the agent can't bypass — http-proxy (GitHub writes + SES email; `km-github` posts direct to api.github.com → transits proxy) and bridge Lambdas (`slack_post`, `h1_comment`; token holders). New `pkg/quota.Record` → atomic `ADD` on new `{prefix}-action-quota` DDB table (+ Streams), multi-window per (sandbox, action): `lifetime`/`hour`/`day` fixed buckets, any-window-trips. Limits resolve per (action,window): profile `spec.limits` → install `km-config.yaml limits:` → unlimited; resolved map → proxy via userdata, bridges via `km-sandboxes` row `action_limits` (must round-trip SandboxMetadata). Phased "alert now, block later": per-limit `onBreach: warn|block|freeze`. Dual notices, both control-plane (uncounted, posted by token holder so a throttled agent can't silence its own warning): operator alert via DDB-Stream→new `km-quota-alerter` Lambda→SES (+ optional Slack control channel), exactly once/breach; user-facing = in-thread (bridge, chat trips) + channel-level (alerter, proxy trips), enforce-aware + idempotent + best-effort. `freeze` = latched quarantine `action_frozen` on sandbox row (survives stop/resume), blocks ALL actions + inbound dispatch refuses turns + frozen notice; triggers = auto-on-breach + `km freeze <sandbox>` CLI (NO Slack trigger); release = `km unlock` ONLY (latch-aware; existing safety-lock intact). Box stays running (latch only; halt/destroy = separate human escalation). `km list`/`status` show FROZEN; `km doctor` surfaces frozen + new table. Deploy gotchas captured: new table in regionalModules()+init.go (make build before km init; module-order test bump); km-quota-alerter needs module+live unit+init.go+lambdaBuilds() (all 4); config key in v2→v merge-list; additive `spec.limits` ⇒ no apiVersion bump. OOS v2: per-actor keying, sandbox-lifecycle storms, OTEL sweeper, sliding windows, Slack freeze trigger. Brainstormed 2026-06-27 from operator question "max PRs/hour + external detection of a malicious chat loop". Design context: `.planning/phases/121-action-quota-and-freeze-quarantine-for-high-impact-outbound-actions/CONTEXT.md`. Depends on Phase 120.

## Session Continuity

Last session: 2026-08-19
Stopped at: Phase 125 EXECUTED — all 9 plans complete, live eight-step UAT run against real AWS (6 PASS, 2 NOT EXERCISED with reasons). Three defects found by live UAT that the green test suite missed, all fixed: km env omitting KM_NAT_GATEWAY_ENABLED (7b2b6af6), checkNATIdle filtering on a never-written DDB status (fa43cf4f), checkPrivateSubnetGuard testing a never-empty slice (d92dbc93). Install returned to public-only; NAT/EIPs released, private subnets retained as routeless islands. Pending: phase verification.
Resume file: None
