...
| Declarative environment definition | Operators want to version and review sandbox config, not imperative scripts | MEDIUM | Fabric's SandboxProfile YAML covers this. apiVersion/kind/metadata/spec mirrors Kubernetes and NVIDIA OpenShell patterns. |
...
| CLI for sandbox management | Operators need scriptable control (CI/CD, local development) | LOW | `fabric create/destroy/list/validate` — standard for any IaC-adjacent tool. |
| Profile validation | Bad configs should fail loudly before provisioning | LOW | `fabric validate` catches schema errors before AWS spend begins. |
| Status / observability | Operators must know if sandbox is running, idle, or crashed | LOW | `fabric list` / `fabric status` + CloudWatch/S3 log destination. |
...
Features that set Fabric apart from generic cloud IDEs, managed sandbox APIs, or DIY Terraform.
...
| Artifact upload on exit | Agents produce outputs (reports, test results, build artifacts). Controlled upload with size limits prevents both data loss and exfiltration. | LOW | Unique to Fabric's design. Other platforms leave artifact handling to the agent. |
...
| AWS resource discovery in ConfigUI | Operators see what AWS resources the sandbox created, not just Fabric's abstraction. Reduces "what did this actually do?" confusion. | MEDIUM | Unique to Fabric's AWS-first design. |
...
| Multi-cloud support | "Avoid AWS lock-in" | Schema is already cloud-neutral. Implementation across AWS + GCP + Azure triples provisioning surface area with no v1 user need. | Keep schema namespace `fabric.run/v1alpha1` cloud-agnostic. Implement AWS only. |
...
| Interactive terminal / IDE in sandbox | "Developers want to shell in and debug" | SSH access into sandboxes creates implicit "pet server" behavior — operators stop treating sandboxes as ephemeral. Conflicts with destroy-on-TTL model. | `fabric status` + audit logs for debugging. SSH is technically possible via EC2 but should not be a promoted feature. |
| Sandbox API server (persistent object store) | "Manage sandboxes via REST API not CLI" | Requires running a control plane service, database, auth, versioning — full platform shift. CLI + Terraform state is sufficient for v1. | `fabric` CLI + Terragrunt state. Add API server in v2 if programmatic access is measured as a blocker. |
...
[SandboxProfile YAML schema]
    └──required by──> [fabric validate]
    └──required by──> [fabric create]
...
- **ConfigUI requires a working CLI:** The UI wraps `fabric` operations. Build CLI first, UI second.
...
- [ ] `fabric validate` — fail fast before AWS spend
- [ ] `fabric create` — compile profile to Terragrunt inputs, provision EC2 + VPC + IAM
- [ ] `fabric destroy` — clean teardown, no dangling resources
- [ ] `fabric list` / `fabric status` — operator visibility
...
- [ ] GitHub source access allowlist — add when Goose-on-Fabric is the primary use case
...
| `fabric validate` | HIGH | LOW | P1 |
| `fabric create` (EC2 provisioning) | HIGH | HIGH | P1 |
| `fabric destroy` | HIGH | LOW | P1 |
...
| `fabric list` / `fabric status` | MEDIUM | LOW | P1 |
...
| Feature | E2B | Modal | Fly Machines | k8s agent-sandbox | Fabric (planned) |
|---------|-----|-------|--------------|-------------------|-----------------|
...
*Feature research for: Policy-driven sandbox / execution environment platform (Fabric)*
*Researched: 2026-03-21*
