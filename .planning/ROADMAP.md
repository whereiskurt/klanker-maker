...
### Phase 84.3: Second-install bootstrap automation (INSERTED)

**Goal:** Make a fresh `git clone` + `km configure` + `km bootstrap --shared-ses --dry-run=false` + `km init --plan` + `km init --dry-run=false` succeed end-to-end against an AWS account that *already* has the shared `sandbox-email-shared` rule set + `sandboxes.${domain}` domain identity + 3 DKIM CNAMEs + MX + verification TXT (the second-install-on-shared-domain case). Today (2026-05-17), an operator running the second install hits six distinct friction points — surfaced live during the `klanker-maker-kph` / `klanker-maker-whereiskurt` UAT after Phase 84.2 closed: (1) `tf-${prefix}-state-${region}` collides with a globally-registered S3 name, terragrunt errors with cryptic 403 HeadBucket only at first apply; (2) `km bootstrap` defaults to `--dry-run=true` and the dry-run text claims "would run: terragrunt plan" when the apply path actually runs `terragrunt apply`; (3) `ExportTerragruntEnvVars` silently honors stale `KM_*` shell env vars over the freshly-loaded `km-config.yaml`, hiding config drift across shell sessions; (4) the foundation auto-detect at `bootstrap.go:204-211` correctly keeps `register_domain_identity=true` for the in-place v1.0.0 → v2.0.0 upgrade path (Phase 84.1 GAP-3 fix) but for the second-install-on-shared-domain case this triggers a re-create attempt against existing DKIM CNAMEs since the foundation has no `import {}` block for them (token-name-not-knowable-at-plan-time, per Phase 84 H9); (5) the documented manual-import workaround in `OPERATOR-GUIDE.md` § Phase 84.1 (already patched in commit `69704cf` for zsh word-splitting compat) is a 4-step ritual every second-install operator must run by hand; (6) `km init --plan` against a fresh install fails on the second module (efs) because `infra/live/<region>/efs/terragrunt.hcl` reads `${get_terragrunt_dir()}/../network/outputs.json` at plan-time, but that file is only written by the apply of the network module — chicken-and-egg: plan can't see upstream outputs that don't exist yet, so the gate's "preview everything before apply" promise breaks for any module that consumes another's outputs.json on a never-applied install; (7) `km configure` writes the literal placeholder string `<prefix>-artifacts-12345678` into `km-config.yaml`'s `artifacts_bucket` field (copied verbatim from `km-config.example.yaml`), so on first apply `km init` tries to upload sidecars to a non-existent bucket and emits a warn-not-fail `NoSuchBucket` error — leaving sandboxes without sidecar binaries at runtime; should derive `${prefix}-artifacts-${account_id}` automatically (matching the `tf-${prefix}-state-${region}` derivation pattern that DOES work); (8) The artifacts bucket is created by `ensureArtifactsBucket` at `bootstrap.go:1411`, but only from the plain `runBootstrap` path (line 1345 — the SCP/KMS/artifacts workflow) — NOT from `runBootstrapSharedSES`. Operators easily skip the plain `km bootstrap --dry-run=false` step thinking `--shared-ses` covers everything (the flag name implies it's an additional option to the same command, not a different sub-workflow). `km configure` should finale with the exact three-command sequence (`km bootstrap --dry-run=false` → `km bootstrap --shared-ses --dry-run=false` → `km init --dry-run=false`), and `km init` should hard-fail (not warn) when the artifacts bucket doesn't exist with a message pointing at the missing bootstrap step; (9) The SCP `aws_organizations_policy.sandbox_containment` at `infra/modules/scp/v1.0.0/main.tf:215-220` is hard-coded with `name = "km-sandbox-containment"` (literal "km-" prefix, NOT `${var.resource_prefix}`), so every install in the same AWS Organization collides on this exact policy name regardless of resource_prefix — `DuplicatePolicyException` from `aws_organizations_policy.CreatePolicy`. WORSE THAN A NAME COLLISION: the SCP's policy *content* also hard-codes "km-" in five trusted-ARN patterns (`scp/v1.0.0/main.tf:14-34` — `km-ecs-spot-handler`, `km-budget-enforcer-*`, `km-ec2spot-ssm-*`, `km-github-token-refresher-*`, `km-ttl-handler`), so even if the second install imports the existing SCP into its state, the SCP attached to the second install's application account will DENY all of that install's lambdas/handlers (ec2:RunInstances, iam:AttachRolePolicy, ssm:SendCommand, etc.) because the deny conditions are `ArnNotLike "km-*-something"` and the second install's roles are named `${prefix}-*-something` (e.g. `whereiskurt-ecs-spot-handler`). The init itself succeeds (operator-SSO is in `AWSReservedSSO_*` which IS trusted), but `km create <sandbox>` later fails: spot handler can't `RunInstances`, budget enforcer can't `AttachRolePolicy`, SSM connections to instances are blocked. Symptom is silent until first sandbox-creation attempt. Unlike DKIM/SES (which are account-shared by design with import blocks for the shared resources), the SCP has no auto-detect or import-on-conflict path: the second install MUST manually `terragrunt import aws_organizations_policy.sandbox_containment <id>` plus the policy_attachment AND THEN either patch the SCP content to add `${prefix}-*` patterns or accept that the SCP enforcement breaks their install. Two architectural options for Phase 84.3: (a) treat the SCP as account-shared (one policy per Organization) and add auto-detect + import to `runBootstrap` mirroring the `detectSharedSESState` pattern, AND make the trusted_arns_* locals consume the union of all known install prefixes (requires an org-level prefix registry — a new DynamoDB table or SSM parameter that all installs read); (b) make the SCP fully prefix-namespaced — `name = "${var.resource_prefix}-sandbox-containment"` AND all trusted_arns use `${var.resource_prefix}` — so each install gets its own SCP attached to its own account. Option (b) is operationally simpler (no registry, no auto-detect, each install's SCP is independent) and matches the Phase 84 prefix-isolation model. Option (a) preserves the "one canonical org-level governance policy" property but requires the registry. The decision needs to be locked in CONTEXT.md. Until Phase 84.3 ships, second-install operators must either detach the SCP from their account (loses backstop) or hand-patch the SCP content to add their install's prefix patterns; (10) Operators recovering from a partial bootstrap (e.g. the SCP collision in (9)) must drop down to direct `terragrunt import` invocations outside the km wrapper, and km provides no helper to export the 12+ `KM_*` env vars (KM_RESOURCE_PREFIX, KM_REGION, KM_REGION_LABEL, KM_DOMAIN, KM_EMAIL_SUBDOMAIN, KM_ROUTE53_ZONE_ID, KM_ACCOUNTS_ORGANIZATION, KM_ACCOUNTS_DNS_PARENT, KM_ACCOUNTS_APPLICATION, KM_ARTIFACTS_BUCKET, KM_OPERATOR_EMAIL, AWS_PROFILE) that `site.hcl` `get_env()` reads and that `ExportTerragruntEnvVars` sets internally. Operators must hand-export each one from `km-config.yaml`, easy to miss any (the user missed `KM_ACCOUNTS_ORGANIZATION` first try, producing a cryptic `arn:aws:iam:::role/km-org-admin` AssumeRole error because the resolved role_arn had an empty account-ID segment — the actual error message blamed permissions, not the missing env var). Phase 84.3 should add `km env` (or `km env --export`) that prints the canonical export block to stdout, so `eval $(km env)` is a one-liner that sets up the operator shell for any direct terragrunt work; (11) `km bootstrap`'s status header displays `Organization account: (not set)` / `DNS parent account: (not set)` / `Application account: ` (empty) even when `km-config.yaml`'s `accounts:` block has all three populated, and the same config-load path produced the correct values on a previous invocation in the same shell. The mismatch correlates with `KM_ACCOUNTS_*` env vars being set between invocations — viper's env-var-vs-yaml precedence interaction is silently shadowing the yaml-loaded account IDs and resolving them to empty (no env-var prefix or wrong-prefix match path). Bootstrap then "Skips SCP deployment — no organization account configured" even though the SCP target IS configured in yaml. Symptom is benign here (operator already imported SCP), but the silent skip hides a real failure mode where Phase 84.3's SCP auto-import path would also no-op for the same reason. Phase 84.3 should: (a) make the config-display banner the source of truth (warn if any account ID resolves to empty post-load, since they're all required), and (b) audit viper bindings to ensure `accounts.organization` etc. yaml keys take precedence over partial / mistyped env-var overrides — or just remove env-var fallback for nested account IDs entirely (the yaml is authoritative; env vars are for terragrunt subprocess consumption only). Phase 84.3 closes all six: pre-flight check at `km configure`, dry-run text correction, env-config drift warning, automatic DKIM/MX/verification-TXT import at bootstrap time when AWS-present-but-not-in-this-install's-foundation-state, downstream-module plan handling on fresh installs (either skip-with-rationale or write-stub-outputs.json on plan-only invocations), and UAT against a real third install (`klanker-maker-rg` style) to prove the path is silent-and-correct end-to-end.

Phase closes when: (a) `km configure` rejects a `state_bucket` value that returns 403 on `aws s3api head-bucket` against the configured profile, prompting for a uniquer name (account-ID-suffix recommended); (b) the dry-run output for `km bootstrap` says "would run: terragrunt apply" (not "plan"), matching the actual non-dry-run behavior; (c) `ExportTerragruntEnvVars` emits a single-line WARN log per drifted env var (e.g. `[warn] KM_RESOURCE_PREFIX=kph in env overrides km-config.yaml whereiskurt — clearing your shell env or running 'unset KM_RESOURCE_PREFIX' will use the config value`) but still defers to the env (no behavior change, just visibility); (d) when `runBootstrapSharedSES` auto-detects that the domain identity exists in AWS but no `aws_route53_record.dkim` entries are in the foundation tfstate, it shells out to `aws ses get-identity-dkim-attributes`, runs 3× `terragrunt import aws_route53_record.dkim[N]` plus conditional MX + verification TXT imports (the same operations as today's manual runbook), and only THEN invokes `terragrunt apply` — so a fresh clone + configure + bootstrap completes in a single command on a shared-domain account; (e) `km init --plan` on a fresh-install regional dir (no module has ever been applied) emits `[skip: depends on <upstream>/outputs.json — apply <upstream> first]` for each downstream module instead of erroring out, and exits 0 with a clean partial-coverage summary OR proceeds with a stub-outputs.json substitution (decision to be locked in CONTEXT.md); (f) a real-AWS UAT against a third install (operator chooses prefix `rg` or similar uniquer string) clones the repo into a fresh folder, runs the standard 3 commands (`km configure` → `km bootstrap --shared-ses --dry-run=false` → `km init --dry-run=false`), and ends with all 17 regional modules + the foundation module green, zero manual `terragrunt import` calls, zero env-var unsetting, zero state-bucket renames.

**Design spec:** `.planning/phases/84.3-second-install-bootstrap-automation/84.3-CONTEXT.md` (to be created during planning — captures the five locked decisions from the live UAT)

**Requirements:** BOOTSTRAP-PREFLIGHT-BUCKET-CHECK, BOOTSTRAP-DRYRUN-TEXT-FIX, ENV-CONFIG-DRIFT-WARN, BOOTSTRAP-AUTO-IMPORT-EXISTING-DNS, PLAN-FRESH-INSTALL-OUTPUTS-HANDLING, ARTIFACTS-BUCKET-DERIVATION, BOOTSTRAP-WORKFLOW-DISCOVERABILITY, SCP-AUTO-IMPORT-OR-PREFIX-NAMESPACING, KM-ENV-EXPORT-HELPER, CONFIG-DISPLAY-VS-YAML-AUTHORITY, THIRD-INSTALL-UAT (synthetic IDs — derived from the eleven problems surfaced in the post-84.2 second-install UAT)

**Depends on:** Phase 84.2 (specifically the `ensureRegionHCL` helper added in commit `c345229` — Phase 84.3 builds on that pattern for the import flow)
