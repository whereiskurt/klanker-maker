...
**Goal:** Claude's Slack output renders like real markdown — proper headings, native clickable link anchors, and actual tables — instead of mrkdwn reflow + monospace grids. Adopt Slack's `markdown` block (GA Feb 2025) and dedicated `table` block (GA Aug 2025), shipped opt-in so the default path and its golden tests stay frozen until a later flip phase.

**Scope:**
1. **Tier-3 `renderRich`** in `pkg/slack/blocks.go`, selected by `KM_SLACK_RENDER=blocks-rich`. Prose segments → Slack `markdown` blocks (verbatim GFM: native bold/italic/headings/lists/code + native `[label](url)` link anchors — this is the "clickable anchors" win, free). Honor the 12,000-char cumulative markdown-block cap with chunking across multiple `markdown` blocks.
2. **GFM-table → `table`-block transformer** (new). Detect contiguous pipe-table runs outside code fences; alignment from the delimiter row (`:--`/`:-:`/`--:`); header row → bold `rich_text` cells; body cells → `raw_number` (pure numeric) / `rich_text` (inline formatting) / `raw_text` (plain). Guards: ≤20 columns, ≤100 rows, pad ragged rows; tables beyond the limits fall back to today's monospace reflow. Reuses existing pipe-table detection.
3. **Structural carryover:** leading H1 still promoted to a real `header` block (markdown-block headings all render same-size); 🔧 tool lines stay `context` blocks; the plain `text` fallback (email/search/push) is unchanged.
4. **`KM_SLACK_AI_FOOTER`** opt-in flag (default off) — appends a "Generated by AI — verify before sharing" `context` footer. No footer for anyone by default.
5. **Fallback chain intact:** `renderRich` → on `ok=false` → existing Tier-2 `RenderBlocks` → Tier-1 `Mrkdwnify`. Existing tiers demoted to fallback, NOT deleted. Default render stays `blocks`, so default-path golden tests do not move.
6. **Tests:** unit coverage for `renderRich` + the table transformer (alignment, cell-type selection, guard/overflow fallback, 12K chunking); new golden fixtures for the `blocks-rich` tier ONLY.
7. **Docs:** `docs/slack-notifications.md` § Phase 111 (render modes, the `markdown`/`table` block adoption, `KM_SLACK_AI_FOOTER`, surface caveats) + `klanker:slack` skill render-mode section + plugin version bump.

**Edge cases (document, not fix):** markdown-block headings flatten to one size (mitigated by H1→header promotion); table cells are not markdown (code spans / nested lists in a cell degrade to `raw_text`); markdown-block images become link text (no inline images — unchanged from today); email notifications + search see only the `text` fallback (tables don't render there, same as today).

**Deploy:** Pure `pkg/slack` + `cmd/km-slack` sidecar change — NO Terraform / DDB / SandboxProfile schema change, NO bridge Lambda change (`PostMessageBlocks` already forwards the pre-serialized block array verbatim). Ship the `km-slack` sidecar binary via `km init --sidecars`; existing sandboxes pick up the new renderer on `km destroy && km create` (or sidecar refresh). Opt-in only — flipping the default to `blocks-rich` is Phase 112.
...
**Goal:** Make `blocks-rich` the default Slack render mode once Phase 111 has soaked on a real workspace. Demote the legacy Tier-2 mrkdwn-section renderer to fallback and reconcile the default-path golden tests in a single reviewed commit.

**Scope:**
1. Flip the default render from `blocks` to `blocks-rich` (the `KM_SLACK_RENDER` default; Phase-68 streaming hook + inbound poller reply pick it up).
2. Reconcile default-path golden fixtures (`pkg/compiler/testdata/*golden*` and any `pkg/slack` goldens) to the new default output — one reviewed commit, clearly diffed.
3. Keep `KM_SLACK_RENDER=blocks|mrkdwn|plain` as explicit operator escape hatches (Tier-2/Tier-1 remain reachable, just no longer default).
4. Docs: update `docs/slack-notifications.md` + `klanker:slack` skill to state `blocks-rich` is now default; plugin version bump.

**Gate (must hold before this phase runs):** Phase 111 UAT passed on a real Slack workspace across desktop + mobile + email-notification fallback — confirm `markdown`/`table` blocks render acceptably and the `text` fallback is sound.

**Deploy:** Same surface as Phase 111 — `km-slack` sidecar binary via `km init --sidecars`; no TF/DDB/schema change. Existing sandboxes pick up the new default on recreate.

**Requirements**: TBD
**Depends on:** Phase 111 (built + soaked).
**Plans:** 0 plans

Plans:
- [ ] TBD (run /gsd:plan-phase 112 to break down — likely a single plan: flip default + golden reconciliation + docs/plugin bump)

### Phase 113: Sandbox self-awareness — on-box profile + capability/network/privilege self-census in klanker:sandbox

**Goal:** Give an on-box agent a single, reliable way to answer *who am I, what can I do, where do I sit on the network, and what is locked down and why* — grounded in both the declarative profile and live runtime probes.

**Scope:**
1. **Platform:** `pkg/compiler/userdata.go` writes the rendered profile to `/opt/km/.km-profile.yaml` at boot (mode 0644, owned by the sandbox user), threading the **same** `remoteProfileYAML` string `km create` already uploads to S3 (`internal/app/cmd/create.go`) — no re-marshal drift. Default verbatim; redact `spec.execution.configFiles` bodies only if spec-review flags an embeddable-secret field.
2. **Skill:** rewrite `skills/sandbox/SKILL.md` from "detect env + email tooling" into a structured self-census — (A) identity & agent, (B) capability census (sidecar helpers / bridges served / channels wired / runtime features), (C) network position (passive checks + exactly one safe-active allowed-vs-blocked egress pair), (D) privilege & restrictions (`sudo -n true` + `privileged` cross-check, git-ref/filesystem enforcement, secret paths, each explained), (E) Slack publish-back readiness (cross-links `klanker:slack`), (F) posture summary. Fix the currently-**broken** `cat /opt/km/.km-profile.yaml` reference (the file now exists) with graceful pre-Phase-113 fallback to env + probes.
3. **Tests/docs:** `pkg/compiler` userdata golden/unit test (profile-write block renders + YAML round-trips); `scripts/validate-all-profiles.sh` stays green; plugin version bump (`plugin.json` + `marketplace.json`).

**Out of scope:** new sidecar binary (`km-whoami`); S3-read IAM grant for the artifacts prefix; any `klanker:slack` content change; full active network mapping (traceroute/port-scan).

**Surface:** `pkg/compiler/userdata.go` (+ template-data struct) and `skills/sandbox/SKILL.md`. NO SandboxProfile schema change, NO new Terraform resource, NO new DDB column, NO bridge Lambda change.

**Deploy:** `make build-lambdas` + `km init --dry-run=false` (create-handler carries new userdata) + plugin version bump. Existing sandboxes gain `/opt/km/.km-profile.yaml` only on `km destroy && km create`; the skill degrades gracefully on pre-Phase-113 sandboxes.
...
4. A shared `pkg/dispatch` helper performs alias-resolution + the resume/cold decision (reusing the existing `DynamoAliasResolver` + sandbox-id-tag handling); the warm/resume path reuses `ttl-handler`'s `handleAgentRun` SSM-dispatch (NO new per-sandbox queue/poller → existing sandboxes need no recreate) and the cold path reuses `PutSandboxCreate`. The GitHub/H1/Slack bridges are not modified (no parity risk).
