...
Plans:
- [ ] 67.2-01-PLAN.md — Wave 1: Add reactionErrorClass + classifyReactionError pure helper + recordingTransport/log-capture test fixtures + TestClassifyReactionError table-driven test (REQ-ACK-RETRY-CLASSIFY)
- [ ] 67.2-02-PLAN.md — Wave 2: Wire classifier into SlackReactorAdapter.Add as bounded retry loop (3 attempts, 200ms→600ms ±25% jitter, Retry-After honoring, ctx-cancellable) + Sleep/Rand injection fields + doOneAttempt/sleepWithCtx/withJitter helpers + 10 new TestReactor_* tests + handler context 5s→10s bump (REQ-ACK-RETRY-BUDGET, REQ-ACK-RETRY-RETRYAFTER, REQ-ACK-RETRY-CTXCANCEL, REQ-ACK-RETRY-JITTER-DETERMINISM, REQ-ACK-RETRY-LOGS, REQ-ACK-RETRY-HANDLER-TIMEOUT)
- [ ] 67.2-03-PLAN.md — Wave 3: docs/slack-notifications.md retry paragraph + CLAUDE.md ACK section update + operator UAT checkpoint (make build && km init --lambdas + live #sb-{id} smoke test) (REQ-ACK-RETRY-DEPLOY)
