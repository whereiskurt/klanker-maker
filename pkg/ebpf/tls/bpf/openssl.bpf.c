                                          ...
                                          __u32 len, __u8 direction, __u32 fd)
{
    ...
    /* Clamp payload length. The BPF verifier needs proof that R2 (the
     * size arg to bpf_probe_read_user) is non-negative and bounded.
     * We force the value through a bitwise AND with a constant mask
     * which the verifier can statically track as an upper bound.
     * MAX_PAYLOAD_LEN is 16384 (0x4000), mask 0x3FFF gives max 16383.
     * We accept losing 1 byte to satisfy the verifier. */
    copy_len = len & 0x3FFF;  /* Force unsigned bound: 0..16383 */
