...
// the plan: ec2spot pins to v1.3.0, ecs pins to v1.0.0, and terraform.source
...
func TestSubstrateVersionPinResolvesPerSubstrate(t *testing.T) {
	...
	if got := versions["ec2spot"]; got != "v1.3.0" {
		t.Errorf("substrate_module_versions[ec2spot] = %q, want v1.3.0", got)
	}
