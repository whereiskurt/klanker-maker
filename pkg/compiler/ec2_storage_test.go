...
func TestHibernationSpotConflict(t *testing.T) {
	...
	_, err := generateEC2ServiceHCL(p, "test-sb", true, nil, nil, "", minimalEC2StorageNetwork(), nil, nil)
	if err == nil {
...
func TestHibernationECSConflict(t *testing.T) {
	...
	_, err := generateEC2ServiceHCL(p, "test-sb", false, nil, nil, "", minimalEC2StorageNetwork(), nil, nil)
	if err == nil {
...
func TestAdditionalVolumeDeviceNameInHCL(t *testing.T) {
	...
	t.Run("sdf occupied in AMI BDMs, picks sdg", func(t *testing.T) {
		hcl, err := generateEC2ServiceHCL(p, "test-sb", false, nil, minimalIAMPolicy(), "", minimalEC2StorageNetwork(), []string{"/dev/sda1", "/dev/sdf"}, nil)
		if err != nil {
			t.Fatalf("unexpected error: %v", err)
		}
...
func TestRawDLAMIAMIPassthrough(t *testing.T) {
	...
	hcl, err := generateEC2ServiceHCL(p, "test-gpu-sb", false, nil, minimalIAMPolicy(), "", minimalEC2StorageNetwork(), nil, nil)
	if err != nil {
