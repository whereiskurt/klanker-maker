...
// ebsnvmeIDScript is AWS's `ebsnvme-id` utility (amazon-ec2-utils, MIT License),
// vendored verbatim so Ubuntu sandboxes — which, unlike Amazon Linux 2023, ship
// neither the AWS udev rules nor ebsnvme-id — can resolve an EBS volume's
// block-device-mapping name (sdf, sdg, …) to its Nitro NVMe node via a local
// NVMe Identify-Controller ioctl. It runs entirely on-box with no network, so it
// is safe under the sandbox's 53/443-only egress SG. The userdata mount block
// writes this to /opt/km/bin/km-ebsnvme-id.py only when the system lacks
// ebsnvme-id (i.e. on Ubuntu). Source:
// https://github.com/amazonlinux/amazon-ec2-utils (file: ebsnvme / ebsnvme-id).
// See docs THIRD-PARTY note + ADDITIONAL_SNAPSHOTS_UBUNTU_MOUNT_FIX.md.
const ebsnvmeIDScript = `
#!/usr/bin/env python3
