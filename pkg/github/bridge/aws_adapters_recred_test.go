...
// path: an @-mention that starts a stopped box must get the same fresh GitHub
// token the operator-driven `km resume` mints, or the agent's first git call
// races an expired credential.
...
// TestEC2Resumer_RefreshFailureDoesNotFailResume is the important one. The
// bridges branch on StartSandbox's error, and on the Phase 109 terminal path a
// failure deletes the sandbox's DynamoDB row and cold-creates a replacement.
// Letting a token-refresh failure surface there would destroy a healthy running
