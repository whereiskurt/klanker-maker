...
type githubInboundDeps struct {
	...
	// DLQArn is the shared (per-install) GitHub-inbound dead-letter-queue ARN
	// (Phase 99.1). When non-empty it is threaded into CreateGitHubInboundQueue,
	...
	// to pre-99.1). Derived from region + account ID + GitHubInboundDLQName(prefix).
	DLQArn string
...
func provisionGitHubInboundQueue(ctx context.Context, deps githubInboundDeps) (queueURL string, err error) {
	...
	queueURL, err = awspkg.CreateGitHubInboundQueue(ctx, deps.SQS, queueName, deps.DLQArn)
	if err != nil {
