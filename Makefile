...
generate-ebpf:
	...
	docker run --rm -v $(PWD):/src km-ebpf-generate:latest sh -c \
	  "cd /src/pkg/ebpf && go generate && cd /src/pkg/ebpf/sni && go generate && cd /src/pkg/ebpf/tls && go generate && cd /src/pkg/ebpf/exec && go generate"
	@echo ""
	...
	@echo "  pkg/ebpf/exec/execbpf_x86_bpfel.go"
	@echo "  pkg/ebpf/exec/execbpf_x86_bpfel.o"
...
build-lambdas: clean bump-version
	...
	GOOS=linux GOARCH=arm64 CGO_ENABLED=0 go build -ldflags '$(LDFLAGS)' -o build/bootstrap ./cmd/km-webhook-bridge/
	cd build && zip -j km-webhook-bridge.zip bootstrap && rm bootstrap
	@echo ""
	...
	@echo "  build/km-webhook-bridge.zip"
