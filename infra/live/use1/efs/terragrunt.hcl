locals {
  ...
  # Read network outputs to wire VPC, subnets, and sandbox SG into EFS inputs
...
remote_state {
  ...
  config = {
    ...
    key            = "${local.site_vars.locals.site.tf_state_prefix}/${local.region_label}/efs/terraform.tfstate"
