...
remote_state {
  ...
  config = {
    ...
    key            = "${local.site_vars.locals.site.tf_state_prefix}/${local.region_label}/dynamodb-capacity/terraform.tfstate"
...
terraform {
  source = "${local.repo_root}/infra/modules/dynamodb-capacity/v1.0.0"
}
