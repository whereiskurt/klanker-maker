...
remote_state {
  ...
  config = {
    ...
    key            = "${local.site_vars.locals.site.tf_state_prefix}/${local.region_label}/dynamodb-identities/terraform.tfstate"
...
terraform {
  source = "${local.repo_root}/infra/modules/dynamodb-identities/v1.0.0"
}
