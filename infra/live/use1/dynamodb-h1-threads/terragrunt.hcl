...
remote_state {
  ...
  config = {
    ...
    key            = "${local.site_vars.locals.site.tf_state_prefix}/${local.region_label}/dynamodb-h1-threads/terraform.tfstate"
...
terraform {
  source = "${local.repo_root}/infra/modules/dynamodb-h1-threads/v1.0.0"
}

inputs = {
  table_name = "${local.site_vars.locals.site.label}-h1-threads"
  tags = {
    "km:component" = "km-h1-inbound"
    "km:managed"   = "true"
  }
}
