...
remote_state {
  ...
  config = {
    ...
    key            = "${local.site_vars.locals.site.tf_state_prefix}/${local.region_label}/dynamodb-action-quota/terraform.tfstate"
...
terraform {
  source = "${local.repo_root}/infra/modules/dynamodb-action-quota/v1.0.0"
}

inputs = {
  table_name = "${local.site_vars.locals.site.label}-action-quota"
  tags = {
    "km:component" = "km-action-quota"
    "km:managed"   = "true"
  }
}
