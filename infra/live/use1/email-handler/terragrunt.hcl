...
remote_state {
  ...
  config = {
    ...
    key            = "${local.site_vars.locals.site.tf_state_prefix}/${local.region_label}/email-handler/terraform.tfstate"
...
terraform {
  source = "${local.repo_root}/infra/modules/email-handler/v1.0.0"
}
