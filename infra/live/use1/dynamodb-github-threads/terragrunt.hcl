...
remote_state {
  ...
  config = {
    ...
    key            = "${local.site_vars.locals.site.tf_state_prefix}/${local.region_label}/dynamodb-github-threads/terraform.tfstate"
...
terraform {
  source = "${local.repo_root}/infra/modules/dynamodb-github-threads/v1.0.0"
}

inputs = {
  table_name = "${local.site_vars.locals.site.label}-github-threads"
  tags = {
    "km:component" = "km-github-inbound"
    "km:managed"   = "true"
  }
}
