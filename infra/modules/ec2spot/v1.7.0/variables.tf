...
variable "sandbox_subnets" {
  type        = list(string)
  description = "List of subnet IDs to launch the sandbox ENI into — public or private depending on spec.network.privateSubnet. If empty, subnets are created in the per-sandbox VPC."
  default     = []
}
...
variable "associate_public_ip" {
  type        = bool
  description = "Whether to assign the sandbox a public IPv4 address. Must be false in a private subnet — a public IPv4 there is unroutable and billable (about $0.005/hr since Feb 2024). Defaults to true to preserve pre-v1.3.0 behaviour."
  default     = true
}
...
# subtracts the two grants opening the secrets bundle. Uid sandbox is blocked from
# 169.254.169.254 by an iptables rule in userdata, and every helper that reads AWS
# as that uid goes through km-creds instead.
#
