...
resource "aws_iam_role" "ec2spot_ssm" {
  ...
  # naming the role's own ARN, and that is not a style choice: IAM resolves a
  # principal ARN to a unique principal id when the policy is SAVED, so a role
  # cannot name itself at CreateRole time. Verified live 2026-09-04 —
  # "MalformedPolicyDocument: Invalid principal in policy". The account root
  # always exists, and aws:PrincipalArn is a global condition key AWS populates on
  # every request, so the two forms are equally narrow. (Contrast the
  # aws:RequestTag trap on ec2:RunInstances, where AWS never populates the key and
  # the condition is unsatisfiable — see the Phase 126 findings.)
  #
  # The role name is spelled out rather than referenced because referencing the
  # resource from its own argument is a Terraform cycle; it is the literal of
  # aws_iam_role.ec2spot_ssm.name a few lines above.
  #
...
# ============================================================
# The identity-based half of the self-assume. The trust statement on the role
# above uses the account-root principal, which DELEGATES rather than authorizes,
# so without this the AssumeRole fails with an AccessDenied naming neither half.
#
# Narrowing happens at RUNTIME, in the inline session policy km-secretsd attaches
# (pkg/secrets.SessionPolicy), not here — which is the whole point of self-assume:
# the narrowed credentials are definitionally this role minus two Denies, so the
# two can never drift the way a parallel role's duplicated policy set would.
