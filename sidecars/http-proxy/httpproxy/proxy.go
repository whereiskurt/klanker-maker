...
// WithDeniedHosts blocks the given hosts unconditionally, using a static list
// fixed for the life of the process. An empty or nil list registers nothing at
// all, leaving the proxy byte-for-byte the same as before this option existed.
func WithDeniedHosts(denied []string) ProxyOption {
	return func(_ *goproxy.ProxyHttpServer, cfg *proxyConfig) {
		if len(denied) == 0 {
			return
		}
		cfg.denier = netpolicy.NewDenier(denied, nil)
	}
}

// WithDenier blocks hosts according to denier, which may combine the static
// profile list with a runtime file the sandbox appends to itself.
//
// The deny gate is registered ahead of every other handler, so a denied host is
// rejected before the Bedrock/SES/Anthropic MITM interceptors, the GitHub repo
// filter, the OpenAI budget path, and the general allowlist ever see it.
//
// The gate is registered whenever denier is non-nil, EVEN IF it currently holds
// no patterns. A runtime deny list legitimately starts empty and grows later, so
// deciding registration from the list's emptiness at construction time would
// mean the first deny a sandbox ever appended silently did nothing.
...
//
// resolver (may be nil) and ctx (may be nil, or carry a nil/empty-RemoteAddr
// Req) are used to opportunistically stamp PID/Comm onto the record — see
// pidResolver in pidresolve.go. Resolution failing for any reason (no
// resolver, no BPF maps under proxy-only enforcement, a map miss, a bad
// RemoteAddr) simply omits PID/Comm; it never blocks or fails the write.
func recordFlow(flows *flowlog.Writer, resolver *pidResolver, verdict, host string, ctx *goproxy.ProxyCtx) {
	if flows == nil {
		return
	}
	...
	if resolver != nil && ctx != nil && ctx.Req != nil {
		if pid := resolver.resolveFromRemoteAddr(ctx.Req.RemoteAddr); pid > 0 {
			rec.PID = pid
			rec.Comm = commForPID(pid)
		}
	}
	_ = flows.Write(rec)
}
...
func NewProxy(allowed []string, sandboxID string, opts ...ProxyOption) *goproxy.ProxyHttpServer {
	...
	// PID resolver for flow attribution (see pidresolve.go). Cheap to
	// construct — it lazily loads pinned BPF maps on first use and fails
	// soft (returns 0/unresolved) when they are absent, e.g. under pure
	// `proxy` enforcement, which pins no BPF maps at all.
	resolver := newPIDResolver(sandboxID)
		...
		proxy.OnRequest().HandleConnectFunc(
			func(host string, ctx *goproxy.ProxyCtx) (*goproxy.ConnectAction, string) {
				...
				log.Info().
					...
					Str("host", host).
					Msg("")
				recordFlow(cfg.flows, resolver, flowlog.VerdictDeny, host, ctx)
		...
		proxy.OnRequest().DoFunc(
			func(req *http.Request, ctx *goproxy.ProxyCtx) (*http.Request, *http.Response) {
				...
				log.Info().
					...
					Str("host", req.Host).
					Msg("")
				recordFlow(cfg.flows, resolver, flowlog.VerdictDeny, req.Host, ctx)
	...
	if cfg.budget != nil {
		...
		proxy.OnRequest(goproxy.ReqHostMatches(bedrockHostRegex)).HandleConnectFunc(
			func(host string, ctx *goproxy.ProxyCtx) (*goproxy.ConnectAction, string) {
				...
				recordFlow(cfg.flows, resolver, flowlog.VerdictRedirect, host, ctx)
	...
	// -------------------------------------------------------------------------
	registerSESMITMHandlers(proxy, sandboxID, resolver, cfg.flows)
			...
			func(req *http.Request, ctx *goproxy.ProxyCtx) (*http.Request, *http.Response) {
				...
				if IsRepoAllowed(repo, allowedRepos) {
					log.Info().
						...
						Str("repo", repo).
						Msg("")
					recordFlow(cfg.flows, resolver, flowlog.VerdictAllow, req.Host, ctx)
					return req, nil
				}
				log.Info().
	...
	if len(cfg.intercepts) > 0 {
		registerInterceptHandlers(proxy, cfg.intercepts, sandboxID, resolver, cfg.budget != nil, len(cfg.githubRepos) > 0, cfg.flows)
	}
	...
	proxy.OnRequest().HandleConnectFunc(func(host string, ctx *goproxy.ProxyCtx) (*goproxy.ConnectAction, string) {
		...
		recordFlow(cfg.flows, resolver, flowlog.VerdictAllow, host, ctx)
