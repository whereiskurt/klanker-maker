...
- **Per-sandbox ed25519 keypair**, auto-generated by `km create` on the operator's laptop.
  Private key stays in `~/.km/keys/sb-<id>`; public key is written to the sandbox at boot.
...
km vscode start $SB
#    Output:
#      ✓ Updated ~/.ssh/config (Host: km-sb-<id>)
...
| Path | Mode | Purpose |
|------|------|---------|
| `~/.km/keys/sb-<id>` | 0600 | Private key (OpenSSH PEM). Never leaves this machine. |
| `~/.km/keys/sb-<id>.pub` | 0644 | Public key (`ssh-ed25519 ... km-sb-<id>`). Shipped via userdata. |
...
- **Per-machine keys (cross-machine create/connect gap).** Keys are generated on the creation
  ...
  different laptop must manually copy `~/.km/keys/sb-<id>*`. Export/import commands
...
| **Key storage** | Private key stays on operator laptop at `~/.km/keys/sb-<id>` (mode 0600) |
