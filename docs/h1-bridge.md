...
| `km create` of an h1-inbound profile fails with `RedrivePolicy is invalid. Reason: Dead letter target does not exist` — **after** the EC2 apply, leaving a running instance on a `failed` row | install predates 2026-09-19: nothing ever created `{prefix}-h1-inbound-dlq.fifo` (module `sqs-inbound-dlq` < v1.2.0). SQS validates the redrive target at `CreateQueue`, not at redrive, so H1 inbound was non-functional since Phase 103. `km init --dry-run=false` (`--only sqs-inbound-dlq` is not in the scoped allowlist), then `km destroy <alias>` to reap the leaked box |
| A missing `@file` event prompt is only a WARN at `km init` and the literal `@profiles/…` string ships as the prompt | known, unfixed (2026-09-19) — the `@file` section above says hard-error; one of them is wrong. Check the prompt path before `km init` |
| `km init` refuses the h1-bridge apply: Lambda env over 4 KB | event prompts are inlined into `KM_H1_PROGRAMS` (command prompts go to SSM). Budget ≈ 1.2 KB + every event prompt's bytes. Shorten the prompt; routing event prompts through SSM is a known follow-up |
