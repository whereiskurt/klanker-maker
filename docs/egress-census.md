...
```
km-netpolicy pin [--dry-run] [--exact] [--yes] [--allow-empty]
```
...
60 minutes; `--max-size` defaults to 250MB with a hard ceiling of 2GB, ring
rotation once hit. There is no flag that raises either ceiling; a capture is a
...
- **A flow producer's file is simply absent** — that source is missing, not
  ...
  no `dns` rows while `http`/`ebpf` rows keep appearing, not a broken command.
