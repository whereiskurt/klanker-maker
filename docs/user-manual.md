...
| `--delete-ssh` | Operator-local `Host {alias} km-{alias} {id} km-{id}` blocks in `~/.ssh/config` and matching keys in `~/.km/keys/` |
...
- `km vscode start` runs an SSM pre-flight check (sshd active + authorized_keys
  present), upserts a managed `Host <alias> km-<alias> <id> km-<id>` block into `~/.ssh/config`, and then
...
2. In VS Code: `F1` → `Remote-SSH: Connect to Host...` → `<alias>` (the sandbox id and the `km-` prefixed forms all resolve too)
