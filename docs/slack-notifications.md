...
| Want the bot to skip 👀 on thread replies (acknowledge top-level engagement only) | **Phase 91.4 (km v0.3.773+).** Set `slack.react_always: false` in `km-config.yaml`, run `km init --dry-run=false`. Top-level messages still get 👀 to signal "I saw you"; thread replies dispatch silently. Per-profile via `cli.notifySlackInboundReactAlways: false` is forward-compat (runtime behaviour today is install-level). |
