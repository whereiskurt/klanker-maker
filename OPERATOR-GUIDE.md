...
### Where the HCL lives, and what is *not* configured by hand

Everything AWS-side in this project is Terraform, driven by Terragrunt. There is
no console step anywhere in the deploy path, and there is no local state file.
Two diagrams cover this end to end —
[`06-terragrunt-invocation.html`](docs/diagrams/security/06-terragrunt-invocation.html)
and [`07-iac-provenance.html`](docs/diagrams/security/07-iac-provenance.html) —
but the short version:

| Path | What it is | In git? |
|---|---|---|
| `infra/live/root.hcl` | Remote-state config plus the `generate` blocks for `backend.tf` and `provider.tf` | yes |
| `infra/live/site.hcl` | Every knob, read via `get_env("KM_…")` — this is where `km-config.yaml` lands | yes |
| `infra/live/<region>/<unit>/terragrunt.hcl` | One unit per module. `km init` walks 28 of them in a fixed, dependency-ordered list (`regionalModules()`) | yes |
| `infra/live/management/scp/` | The org-level Service Control Policy | yes |
| `infra/modules/<name>/v<X.Y.Z>/` | Versioned module bodies. Immutable — a change is a **new directory**, never an edit in place | yes |
| `infra/templates/sandbox/` | The per-sandbox `terragrunt.hcl` + `service.hcl` skeleton, copied per `km create` | yes |
| `infra/live/<region>/region.hcl` | Written by `km init` on first run (`ensureRegionHCL`) | **gitignored** |
| `infra/live/<region>/sandboxes/<id>/` | Compiled per sandbox: `service.hcl` + `user-data.sh` from the SandboxProfile | **gitignored** |
| `.terragrunt-cache/**/backend.tf`, `provider.tf` | **Generated by Terragrunt at runtime**, every run | **gitignored** |
| `infra/live/**/outputs.json` | Cached module outputs for downstream units | **gitignored** |

Three things people reliably assume are manual, and are not:

1. **`backend.tf` and `provider.tf` don't exist in the repo** because `root.hcl`
   emits them with `generate` blocks (`if_exists = "overwrite_terragrunt"`) into
   the run's `.terragrunt-cache/`. Grepping the tree for a backend block finds
   `root.hcl`, and that is the only definition.
2. **Nothing is typed into the AWS console.** `km` never calls terraform
   directly; `pkg/terragrunt.Runner` execs `terragrunt apply -auto-approve` with
   `cmd.Dir` set to one unit directory and the `KM_*` variables in the
   environment (`TG_BACKEND_BOOTSTRAP=true`, `TG_NON_INTERACTIVE=true`). There
   is no `run-all` — units are applied one at a time, in a declared order, so a
   failure stops at a nameable module.
3. **State is remote, always.** Per region:

   ```text
   bucket   tf-{prefix}-state-{regionLabel}     e.g. tf-km-state-use1
   lock     tf-{prefix}-locks-{regionLabel}     e.g. tf-km-locks-use1
   key      tf-{prefix}/{regionLabel}/{unit}/terraform.tfstate
   ```

   Both are created by Terragrunt's backend bootstrap on the first apply against
   a fresh account. Cross-account capacity links get their own backend in the
   *target* account: `tf-{prefix}-linkstate-{target-account-id}-{regionLabel}`.

The one file a human writes is `km-config.yaml` (gitignored, per install), plus
the SandboxProfile YAML for whatever they want to run. `km configure` will
generate the first for you.
